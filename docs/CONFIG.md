...
| `voice_id_enabled` | `true` | Room mode's offline local speaker matcher (#28) - **the** identity path since the cloud fallback retired. On, a known voice is named on-device in a fraction of a second; a voice the matcher cannot place stays honestly unnamed, and the only ElevenLabs batch call left runs when voices overlap (crosstalk splitting). Off (`CROSSBAND_VOICE_ID_ENABLED=false`), or when the `sherpa-onnx` wheel or the model file is absent, turns are simply not named and room mode never arms automatically - introductions, spoken commands and the switch in the voice settings still arm it by hand. Tuning guidance: [VOICE_ID.md](VOICE_ID.md). |

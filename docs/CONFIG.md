...
| `browser_origin` | `""` | Where a browser opens this app, for the other apps to link here. Empty works it out: `https://` and the first trusted host when there is one, else `http://127.0.0.1:8902`. Set it when Tailscale serves the app somewhere else, such as its own port. |
| `sibling_apps` | Membro, Spendglass and Threadfold on their usual ports | Your other apps, linked from a row at the top of every page. Each entry is a name and the health address that app answers on this computer. One that doesn't answer isn't shown, and `{}` turns the row off. [REMOTE_ACCESS.md](REMOTE_ACCESS.md#the-other-apps-on-the-same-tailnet). |

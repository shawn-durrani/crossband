...
- (`test_person_sync.py`) The membro person sync (#33): the first pass backfills local people and clips (re-runs are no-ops by content-addressing), a person membro holds is rebuilt locally, a forgotten mark deletes the local copies, participant-named entries are never pushed, no-token/unreachable are clean no-ops - and the correction replay: local moves, deletes and merges land in membro (never resurrecting through a rebuild), survive membro being down, and a pull never clobbers a survivor's mapping with a duplicate record
...
- (`test_memory_attribution.py`) Guest speaker classes on memory ingest (#28 phase 3): owner turns stay `user`, confident guests go `guest:<preferred name>`, uncertain and open-flagged turns go `guest:unknown`, `SOURCE_APP` untouched, and the contract-1.2 identity beside the label (#33): confident guest turns carry {person, confidence, method} built from the label's real provenance and stamped matcher score, everything weaker stays string-only
...
- (`scripts/render-smoke.mjs`, gating `npm test` and CI) The real ThreadView + Message rows render against realistic fixtures (typed, voice, system, streaming, usage-bearing turns) under react-dom's server renderer - any render-time crash fails the suite instead of blanking every chat in production; proven to catch the 2026-08-14 regression
- (`eslint src`, gating `npm test` and CI) no-undef across the whole frontend: a free identifier in JSX passes the bundler and every pure-module test, then blanks the app at runtime - the 2026-08-14 blank-chat regression, twice in one day
- (`voiceSurface.test.js`) Voice survives page switches (#69): a live session swaps its call surface for the compact strip while a full page is open, and never ends because a panel opened
- (`voiceDiscard.test.js`) The discard affordance (#106, #111): only the owner's settled voice turns qualify, the confirm copy states exactly what a discard cannot undo, and the erase link names membro on the same host with an encoded ref (or honestly nothing on an older server)

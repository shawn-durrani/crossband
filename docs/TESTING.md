...
- (`test_voice_clips.py`) Clip audition (#68): metadata-only list newest first, audio served only for the person's own file tokens (traversal and cross-person refused), playback read-only for anchor state, single-clip delete recomputes sufficiency, last clip's deletion leaves the person known but unlearnt
...
- (`voiceClips.test.js`) Clip audition rows (#68): every capture path has a plain-English label, unknown sources render verbatim rather than guessed, only elimination-earned clips get the closer-listen flag, row derivation keeps the file token and formats duration, and the delete explainer says what actually happens

...
- **`ttl`** — always `5m-ephemeral-default` today (`providers.CACHE_TTL_LABEL`
  — a constant, not a per-call measurement). It exists so a future TTL change,
...
richer view — it splits stable/volatile/transcript by hash and shows which
TTL bucket a write landed in — but it is a sampling tool, not the only record.
...
2. **TTL expiry.** The *same* `stable_hash` on two calls more than ~5 minutes
   apart still costs a fresh cache write — ephemeral breakpoints expire. A
...
- **`cost` may be `None`.** A utility model absent from the local pricing
  table is recorded as "not tracked", never a silent `$0.00` — same
...
- **`provenance` is recorded at write time, immutably** (schema v11). The cost
  ...
  resolved once, when the call happens, and stored on the row — a later edit
...
future change"), and use a comparable conversation for both samples — same
...
1. **Turn on verbose logging for the session.** `config.local.json` is the
   ...
   you run directly or under the launchd supervisor — no need to touch the
   supervisor's plist:
   ```json
   { "log_level": "INFO" }
   ```
...
5. **Compare structurally, not as a dollar claim:**
   ...
   - Report the structural comparison (hash churn, read/write ratio) — not a
     percentage or dollar figure — unless you've independently confirmed the

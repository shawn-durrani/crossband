...
Run these from the repo folder — `tail` and the installer are relative to it
...
own — the database is the catch-up buffer, so nothing is lost across a restart.

## Turning up log verbosity temporarily

By default only `WARNING`-and-above from the app's own code reaches
`data/service.log` — that includes the content-free Claude-chat cache
...
and `WantedBy=default.target`. A unit file isn't shipped here yet — the

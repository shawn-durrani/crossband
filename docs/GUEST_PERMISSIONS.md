...
coding agent, a program that reads a repo, edits its files and runs
commands for you. You can summon it into a chat with the
`summon_claude_code` tool, and it joins as a guest for one turn, in one
repo, and then it leaves. There's no terminal and nobody to click
"approve", so everything the guest may do is decided in code before it
starts, and every visit gets the same rules. Read this before you turn
on implement mode, and before you change the lists in
`backend/guest.py`.
...
The guest never works in your checkout. Each visit gets its own
[git worktree](https://git-scm.com/docs/git-worktree), a second copy of
the repo in a temporary folder that shares the repo's history, branches
and remotes but keeps its own files on disk. The worktree is keyed by
the repo, the chat and the visit, so two visits never share files, and
a visit can't collide with you or with another chat's guest. Before a
visit starts, the app fetches from origin and checks the worktree out
at the newest `main`, or at your local `main` when there's no remote. A
summons can also name a branch or a pull request, and then the worktree
is checked out at that commit before the guest starts, so a review
...
and open a pull request. It can never merge and can never push to
`main`. You review and merge every pull request.
...
split by party, producer and provider.

The tool is always on, while the servers in `code_mcp` are opt-in,
because it carries no secret. Its answers come from the running app's
own data, and it reaches no network. It can't return a transcript, a
message, a credential or a log line.

Claude and GPT have the same tool in normal chat, so a diagnostics
question doesn't need a guest, and a guest summoned for something else
still has the tool. The guest's copy is `backend/diag_mcp.py`, the
chat's is in `backend/tools.py`, and both share one list of names, one
schema and one dispatch in `backend/diagnostics.py`.

### Choosing a model and an effort level

A summons can name a model tier and an effort level. Ask for either in
the chat and the model that summons the guest passes it along, or set
`code_model` and `code_effort` in config for every visit. The tiers are
`default`, `opus`, `sonnet` and `haiku`, and the effort levels are
`default`, `think`, `think-hard` and `ultrathink`. Both are fixed
lists, checked when the tool is called, so nothing free-form reaches
Claude Code. A choice in the summons wins over the two config keys, and
those win over Claude Code's own default.
...
budget, and nothing confirms it, because Claude Code never reports
thinking tokens back.

## How a command is approved

A command runs only if a rule pre-approves it, and anything else is
denied on the spot. Every guest session runs with
`permission_mode="dontAsk"`, the
[Claude Code permission mode](https://code.claude.com/docs/en/agent-sdk/permissions)
that turns a request for approval into a refusal, because nobody is
there to answer one. That's why the allow list has to name the
...
suite pins which rules are handed over and never sees a command
refused.
...
| GitHub, through its command-line tool [`gh`](https://cli.github.com/manual/) | `gh issue view`, `gh issue list`, `gh issue comment`, `gh pr create`, `gh pr view`, `gh pr checks`, `gh pr diff`, `gh pr list`, `gh pr status` and `gh auth status`. |
...
The keyless test command in `CLAUDE.md` starts with
[`env`](https://man.freebsd.org/cgi/man.cgi?query=env&sektion=1), so
it has its own rule, in both key orders. `env -u NAME` only unsets a
variable before the pinned interpreter, so the rule stays as narrow as
`.venv/bin/python` itself.

[`sqlite3`](https://sqlite.org/cli.html) is SQLite's command-line
shell, and the rules approve only its `-readonly` and `-ro` forms,
which open the database read-only, so a stray `UPDATE` or `DROP` fails
in the engine. A bare `sqlite3 <db>`, which can write, is never
approved.
...
runs, and SQLite's `-readonly` still allows dot commands like `.shell`
and `.import`, which no rule covers. The gate that catches all of this
...
the protection binds while you're there, because these settings often
exempt the repository owner and bind anyone pushing as a collaborator,
which is the account a guest pushes under.
...
damage is where the guest stands, since a visit runs in a fresh
...
`WebFetch`, `WebSearch`, `Task`, `TodoWrite` and `KillShell`. It holds
...
The rules decide what may run, but they don't log the guest in
anywhere. `git push` over an HTTPS remote and every `gh` command need a
GitHub credential. If the host blocks Keychain access, which is common
when Crossband runs in a restricted environment, they fail with an
HTTPS error, `OSStatus -26276`, however the permissions are set.
...
Claude and does nothing for GitHub.
...
- `CLAUDE_CODE_OAUTH_TOKEN` stays, since it's the supported headless
  login, made with `claude setup-token` and kept in `.env`. Blank it
...
The usual first entry in `code_mcp` is the `membro` recall server of
[Membro](https://github.com/shawn-durrani/membro), the memory service,
and it offers recall and `save_memory`, a fact proposal that waits for
your review. Add it in `config.local.json`, so your paths never enter
the public repo.
...
guest launches, so a secret is named in config and never pasted into
it. Put the real value in Crossband's `.env`, as `MEMORY_AUTH_TOKEN=…`.
`"Bearer ${TOK}"` works as well as a bare `"${TOK}"`. The replacement
works in `env` only, so a `${VAR}` in `command` or `args` is passed
through as written and never resolves. A variable that isn't set
becomes an empty string, so a typo leaves the tool with no credential
at all, and the literal `${VAR}` never leaks.
...
otherwise. It never grants write, approve or dismiss, which stay
owner-only by the server's own rules.
